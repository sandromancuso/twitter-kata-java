...
public class UserRepositoryShould {
	...
	return_just_a_users_post_when_user_does_not_follow_anyone() {
	    ...
	    assertThat(posts.size(), is(1));
	    assertThat(posts.get(0), is(new Post("Alice", "Hi, I'm Alice")));
	}
	...
	return_posts_from_the_users_that_a_specified_user_follows() {
	    ...
	    assertThat(wallPosts.get(0), is(new Post("Alice", "Hi, I'm Alice")));
	}

	@Test public void
	return_posts_from_users_and_users_she_follows_in_reverse_chronological_order() {
		clockReturns(todayAt(10, 00, 00), todayAt(10, 00, 01), todayAt(10, 05, 10));
	    ...
	    assertThat(wallPosts.get(0), is(new Post("Charlie", "Hi, I'm Charlie")));
	    assertThat(wallPosts.get(1), is(new Post("Bob", "Hi, I'm Bob")));
	    assertThat(wallPosts.get(2), is(new Post("Alice", "Hi, I'm Alice")));
	}

	private void clockReturns(Date... dates) {
		Date head = dates[0];
		Date[] tail = (dates.length > 1) ? copyOfRange(dates, 1, dates.length - 1) : new Date[] {};
